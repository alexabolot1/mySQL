...
SELECT * FROM catalogs where id in (1,2,5) order by FIELD(id, 5,1,2);
...
select * from users;
select avg(age) from (select name, floor((to_days(now()) - to_days(birthday_at))/365.25) as age from users) as avg_age;
